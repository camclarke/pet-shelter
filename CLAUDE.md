...
| **Outbreak trace — UI** | ✅ **Built 2026-08-24** — on the animal's internal page: pathogen + diagnosis date. `noPlacementData` renders as its OWN message, never as "sin contactos" |
...
- **2026-08-23** — **Built step 6, the re-admission / dedup path — and the thing worth keeping is that it was fully verifiable with `pets` holding zero documents.** Plan §3.1. Entering a valid chip at intake now runs a lookup *before* the form is trusted: unknown → new intake, known → offer to reopen the existing record, "es otro animal" → the conflict is recorded and publishing that code onto a second record is blocked. Built before the first real animal deliberately, because a dedup key is cheap while the collection is empty and expensive the moment a duplicate exists. **The lookup is automatic rather than a button**, which is a design decision and not a convenience: a dedup check that has to be remembered is a dedup check that does not happen, and the cost is one Firestore read per *completed valid* code — keyed on the normalised value, so retyping `068 0000…` as `0680000…` does not re-query. **Three findings.** (1) **No rules change and no index change were needed, and that was checked rather than assumed.** `firestore.rules` already carried `match /{path=**}/identity/{docId} { allow read: if isAdmin(); }` and the `identity.code` collection-group `fieldOverride` was already deployed — the decision to let an admin client run this query had been made months earlier and simply never used. What *was* wrong was the comment above it, which said the group was "queried on the server via the Admin SDK; from a client it stays shut." **That was never what the rule did** — only what the code happened to do at the time. A comment describing current usage as though it were policy is worse than no comment, because the next person reads it as the boundary. Rewritten to name both callers, redeployed, and read back **byte-identical** in Node — never PowerShell, whose `Get-Content -Raw` mis-decodes these BOM-less UTF-8 files. **Normalise line endings on BOTH sides before comparing, and say which measure a byte count is.** The live `firestore.rules` is 17065 UTF-8 bytes *with* CRLF and 16744 *without*, and the working tree is CRLF — so an unnormalised diff reports a 321-byte mismatch that is 321 `
` characters and nothing else. Worse for `storage.rules`: the live copy is **LF** while the local one is CRLF, because `scripts/release-storage-rules.mjs` sends normalised content whereas `firebase deploy` uploads the tree verbatim. **A naive byte comparison of `storage.rules` will therefore ALWAYS report drift on this machine, forever.** (2) **`findPetByMicrochipAdmin()` reads `limit(2)`, not `limit(1)`.** The server-side finder has always used `limit(1)`, which cannot distinguish "one animal has this chip" from "two do" — and silently returning the first is precisely wrong for a credential by which ownership is asserted. The verdict is a three-way union (`unregistered` / `registered` / `ambiguous`) so the impossible case surfaces to a human instead of being resolved by query order. (3) **"Es otro animal" had to block, not warn.** Letting it through would write one chip number onto two `identity` documents, at which point `findPetByMicrochip()` — the finder's path, the single most important query in the system — resolves a scanned chip to whichever record the query returns first. So the conflict is recorded **on the draft** (persisted, so it survives a reload) and gates publishing via a new `IntakeError`. It clears itself when a *different* code is entered, because a transposed digit is by far the likeliest cause and a gate that stayed shut after the correction would only teach people to uncheck the box. A failed *lookup*, by contrast, warns and does **not** block — plan §3 is explicit that a gate stricter than the shelter's reality gets worked around, and the workaround is the WhatsApp group this system exists to replace. **The verification is the point, and it went further than expected.** A 26/26 client-SDK probe against live Firestore proved the collection-group query resolves for an admin **and is denied for a signed-in non-admin** (both branches, as always), that the re-admission closes the previous custody interval and leaves **exactly one** open, that `merge: true` does not drop `createdAt` — which `getWall()` orders by, so a plain `set()` would have looked like the animal vanishing — and that medical and identity are untouched. Then the wizard was driven in a browser end to end. **Four things only the browser could show.** (a) **My own fixture chip was rejected, and the validator was right**: `068900000000042` has a national ID of 900,000,000,042, past the 38-bit ceiling of 274,877,906,943, so it is not a physically possible ISO 11784 code — the form refused it and correctly ran no lookup at all. (b) Renaming the animal back to a *former* name produced `name: 'Nube'`, `formerNames: ['Luna']` — the chain removed the revived name rather than duplicating it, which is the one branch of `nextFormerNames()` that is easy to get subtly wrong and impossible to notice later. (c) **The dossier rendered with a chipped, renamed pet for the first time ever**, and the Spanish participles read *"Antes **conocida** como Luna"* and *"Está **identificada** con microchip"* — so the `pastParticiple()` fix from earlier today is now proven against real data rather than merely corrected in source. That copy is untestable by construction; it only exists when a pet does. (d) The wall stayed empty while a `quarantine` animal existed, exercising the `status == 'available'` allowlist against real data a second time. **Two smaller notes.** The browser pane was not compositing, so screenshots and *scrolling* both failed — `document.scrollingElement.scrollTop` stayed 0 against a 2093 px page. The way through is `resize_window` to a viewport taller than the document; ref-based clicks and `read_page` keep working, and `read_page` only reports elements in the viewport, which is why the form looked truncated. And **button text matched by `innerText` comes back CSS-uppercased**, so `/otro animal/` finds nothing and `/otro animal/i` does — a false negative that looks exactly like a missing element. All probe data removed and read back at zero: 0 pets, 0 drafts, 0 in every collection group, and the only auth account is the real admin. 103/103 tests, typecheck clean, build clean, 0 vulnerabilities in both trees.

- **2026-08-23** — **PR #13 merged and deployed; step 6 is live.** The deploy ran on `f2f2042` and the running Cloud Run tag **equals `git rev-parse HEAD`**. All eight routes return 200 on both hosts with `s-maxage=300, stale-while-revalidate`, so nothing inherited the year-long Hosting cache defect. Both rulesets read back matching `main`. **The production check found no defects — but three of its own assertions failed first, and all three were the probe.** That is the fourth recurrence in this project of *the test was broken, not the thing under test*, and two of the three are permanent traps rather than one-off slips, so they are worth writing down. (1) **A needle grepped out of source may be a code COMMENT.** "Reabrir expediente" appears only in a JSDoc block; the minifier strips comments, so it is correctly absent from the bundle and its absence proves nothing. A third needle failed on capitalisation — `Ya estuvo aquí` is lowercase mid-sentence in the JSX. **Take bundle needles from rendered JSX and match their exact case, or the probe reports a healthy build as broken.** (2) **Grepping the homepage for `firebase` is a FALSE POSITIVE, permanently** — and this file's own instruction to "fetch the homepage's chunks and grep" is what leads into it. The initial chunk legitimately contains `getFirebase` (the dynamic-import helper's name) and a destructured `onAuthStateChanged` binding, because those are `AuthProvider`'s **call sites**; the SDK itself sits behind webpack's `e.A(99939)` lazy loader. The fingerprints that actually distinguish the SDK from a reference to it are **`initializeApp`, `@firebase/auth`, `identitytoolkit`, `signInWithPassword` and `FirebaseError`** — all five absent, while the homepage stayed at 9 chunks / 635 KB against the 634 KB recorded earlier. The split holds; the old check was just too coarse to show it. (3) **Comparing a deployed ruleset to the file needs line endings normalised on BOTH sides, and a byte count means nothing without saying which form it counts.** `firestore.rules` is 17065 UTF-8 bytes with CRLF and 16744 without — an unnormalised diff reports a 321-byte mismatch that is 321 `
` characters. `storage.rules` is worse: the live copy is **LF** and the local one **CRLF**, because `release-storage-rules.mjs` sends normalised content while `firebase deploy` uploads the tree verbatim, so a naive byte comparison of that file will report drift on this machine forever.

- **2026-08-24** — **Built step 5a, the arrival pipeline UI — and the outbreak ledger finally has a writer.** `placements.ts` and the trace were built and proven against live Firestore on 2026-08-16, but **nothing in the product had ever written a placement**: `IntakeWizard.tsx:115` already offered `status: 'quarantine'` while `grep` found no reference to `areas` or `placements` anywhere under `src/app/` or `src/components/`. So an animal could be marked quarantined and the system recorded *nowhere it had been*. Two screens close that: `/admin/areas` (occupancy board + area editor) and `/admin/pets/{petId}` (current pen, full history, move / clear / release, and the trace). `src/lib/areas.ts` is pure and testable offline, `src/lib/areas-admin.ts` does the Firestore work client-side under the rules — the same split as `intake.ts`/`pets-admin.ts`. **Neither `firestore.rules` nor `firestore.indexes.json` needed a line changed**, and that was checked rather than assumed: `match /areas/{areaId} { allow read, write: if isAdmin(); }`, the admin-only `collectionGroup('placements')` rule, and both `placements(areaId, …)` composite indexes were all written on 2026-08-16 and confirmed **READY** by `gcloud` before any code leaned on them. Second time this project has found a decision already made and merely uncalled — the `identity` collection group was the first.

  **The live probe found two real defects in the new code, and both were clock skew.** A 22/22 client-SDK run against live Firestore failed three assertions first. Measured rather than guessed: a `serverTimestamp()` written and read straight back showed **Firestore's clock 2.7 s AHEAD of this machine**. Consequence one was cosmetic — `daysSinceLastArrival` came out **-1**, because `Math.floor` of a tiny negative is -1, so a pen would have read *"último ingreso hace -1 días"*. Consequence two was not: the exposure window's ceiling is the **caller's** `now` while every `startedAt` is **Firestore's**, so a placement written seconds earlier sorted *after* the window ended, was clipped out of the subject's own stays, produced no areas, and **the trace returned zero contacts** — with `noPlacementData` false, so the UI would have reported "sin contactos" as a confident answer about an animal that had just been put in a pen with two others. That is the exact silent failure this subsystem is arranged to prevent, arriving by a route nobody had considered. Fixed with `CLOCK_SKEW_TOLERANCE_MS` (five minutes, not five seconds — a browser clock is what will ask this, and browser clocks drift by minutes) applied in **both** trace implementations, plus a clamp on the day count. Each fix carries a regression test written in the units the failure happened in, and **both were proven to fail when the fix is reverted**.

  **A third defect was visible only in a browser.** The trace rendered an ongoing contact — two animals *both still in the pen*, the most urgent row in the list — as `24 ago 2026 → 24 ago 2026`, which reads as finished. Cause: `traceContacts` clips the subject's stay to the window before comparing, which turns an open stay into one ending at the window's edge, so the documented `overlapEnd: null` ("both animals are still in there") was **unreachable**. The clip exists to *measure* exposure, not to decide whether it is over. `overlapEnd` is now null when neither animal has left, and the row reads *"→ sigue"*.

  **The deliberate-break probe earned its keep twice.** Breaking each rule in `areas.ts` on purpose showed two were covered by **no test at all**: an infectious move into a **quarantine** pen (only the general pen was tested — and quarantine is the case that matters, because that pen is full of healthy newly-arrived animals), and `statusAfterPlacement`'s legality guard. The first was a real hole and now has a test. The second turned out to be **unreachable defensive code** given today's transition table — so rather than pretend it was covered, the *coupling* it protects is now asserted directly: narrowing `PET_STATUS_TRANSITIONS` so `inbound` can no longer reach `quarantine` fails a test **by name**, which was verified by doing it.

  **Verified in a browser end to end**, not merely compiled: the empty state's "no inventes nombres bonitos" guidance; both validation errors on an empty form; the **duplicate-name guard firing on `CUARENTENA  2` against an existing `Cuarentena 2`** (case *and* doubled spacing — two rows for one pen splits occupancy, and an `areaId` trace then silently misses everyone filed under the other spelling); the status hint *"Al guardar, el animalito pasa a «En cuarentena»"* shown **before** the move; the ASV warning and the cohorting warning appearing together against real occupancy with the save button **still enabled**, because these warn and never block; the board reading `2 de 2` with *"Último ingreso hoy"* (the clamp, in the browser); `0 animalitos` for an uncounted capacity; attribution resolving to the mover's email; the trace finding the right animal in the right pen and excluding the one that was never there; and the `sin área` flag appearing on a quarantined animal once its placement was closed. Zero console errors.

  **Two smaller findings.** (1) **`export const revalidate = 300` is inert on a dynamic route**, measured from a real `next start`: `/admin/pets/{id}` — and the pre-existing `/adopt/[slug]`, which carries the same export — both answer `private, no-cache, no-store`. The year-long Hosting cache defect applies to **static** pages only. The line stays, because it becomes load-bearing if the route ever prerenders, but the comment claiming it prevents a freeze was corrected: a comment describing intent as though it were behaviour is the mistake this file recorded about a rules comment one day earlier. (2) **Python's text-mode read translates CRLF to LF**, so every patch script silently rewrote its file as LF against a CRLF working tree. Git normalises on commit so nothing was at risk, but it warned on every command until the endings were restored — and `git diff` then showed `arrival.ts` as *modified with no content change*, which is what proved a break-probe restore was clean. **The heredoc lesson also recurred a fourth time**: a `python - <<'PY'` heredoc ate a backslash and died on a syntax error. Write the script to a file and run the file.
...
- **2026-08-27** — **Secret Manager went from empty to bound, in the three
  separate applies the design demanded, and the first secret this project has
  ever held is live.** `gemini-api-key` version 1, enabled, **39 bytes** —
  checked by byte count rather than by reading the value, which is enough to
  catch the failure that mattered (a pipe delivering nothing would have been a
  0-byte payload and a silently unconfigured app).

  **Cloud Run revision `00017-qf9` booted Ready with the `secretKeyRef`, and
  that is a stronger claim than it looks.** Cloud Run refuses to start a
  revision whose secret reference it cannot resolve, so a revision reaching
  Ready is the first *exercise* of the `secretmanager.secretAccessor` grant
  that has sat in `iam.tf` unused since 2026-08-07. Granted and working are
  different claims, and this is the one that earns the second. The image stayed
  `365cbd0…` throughout, so `lifecycle { ignore_changes }` held through an
  apply that modified the very resource it guards.

  **The gate did its job and was measured doing it.** Step 1 applied with
  `gemini_api_key_enabled = false`: 1 to add, 1 to change, and the Cloud Run
  diff was *only* the benign `scaling` no-op with the env list untouched —
  confirmed by reading the env list back afterwards, which held exactly one
  entry. Step 3 was applied from a **saved plan file** rather than a re-plan, so
  what was applied is what was reviewed, and that plan showed the `+ env` block
  with `secret = "gemini-api-key", version = "latest"` and nothing else.

  **⚠️ The one real hazard this created is that the flag is gitignored.**
  `gemini_api_key_enabled = true` lives in `terraform.tfvars` and nowhere
  else. A fresh clone copies `terraform.tfvars.example`, where it is
  `false` — so an apply from another machine would plan to **REMOVE** the env
  binding and silently un-configure production, with no error and no obviously
  wrong-looking diff. It is the inverse of the CI-injected-var lesson already
  in this file: there a variable Terraform did not declare got deleted; here a
  variable Terraform *does* declare, but cannot see the value of, does.

  **What this does NOT earn, and the number still says so:**
  `api_usage_daily` is **still 0**. The key reaching the container is not a
  call. `/api/intake/suggest` returning something other than 503 to a signed-in
  admin is **unverified**, and unverifiable from outside, because the 401 is
  deliberately returned *before* the configured-check — so no unauthenticated
  probe can see past it. Verified instead: all 20 route/host combinations 200,
  the auth boundary still 401 / 401 / 405 on both hosts, and the homepage still
  9 chunks / 635 KB with `GEMINI_API_KEY`, `generativelanguage` and
  `createGoogleGenerativeAI` all absent — binding a secret to the server did
  not leak one to the browser.
...
5. ~~**Production `GEMINI_API_KEY`.**~~ — **DONE 2026-08-27.** All three steps
   applied. `gemini-api-key` holds version 1 (enabled, 39 bytes) and Cloud Run
   revision **`00017-qf9`** booted Ready carrying it as a `secretKeyRef`, not a
   plain value. **The revision reaching Ready is itself the proof the IAM grant
   works** — Cloud Run refuses to start a revision whose secret reference it
   cannot resolve, so a booted revision means `secretmanager.secretAccessor`
   was genuinely exercised rather than merely granted. Image unchanged
   (`365cbd0…`), so `lifecycle { ignore_changes }` held through an apply that
   touched the same resource.

   ⚠️ **THE FLAG LIVES ONLY ON THIS MACHINE.** `gemini_api_key_enabled = true`
   is in `terraform.tfvars`, which is **gitignored**. Anyone applying from a
   fresh clone copies `terraform.tfvars.example`, where it is `false` — and
   Terraform would then plan to **REMOVE** the env binding and silently
   un-configure production. That is the inverse of the CI-injected-var lesson
   this file already carries: there, a var Terraform did not declare got
   deleted; here, a var Terraform declares but cannot see the value of does.
   **Check `gemini_api_key_enabled` before applying from any new machine**, and
   read a plan proposing to remove that `env` block as a missing tfvars entry,
   never as intended drift.

**The key now reaches Cloud Run, so the feature is switched ON in production —
and that still is not the same as it having worked.** What is proven: the
secret resolves, the revision boots, the container receives the value as an
env var. What is NOT proven: that `/api/intake/suggest` answers anything other
than 503 to a signed-in admin, because that needs a sign-in and the 401 is
deliberately returned BEFORE the configured-check, so no unauthenticated probe
can see past it. The auth boundary was re-verified after the bind:
**401 / 401 / 405** on both hosts, all 20 route/host combinations 200.

Admitting an animal still never depends on it, by design: plan §3 holds that a
gate stricter than the reality of the shelter gets worked around.
...
| **Prod bundle split** | fetch the homepage's 9 chunks and grep for `initializeApp` / `@firebase/auth` / `identitytoolkit` / `signInWithPassword` / `FirebaseError` | ✅ **all five absent, 9 chunks / 635 KB** — the `AuthProvider` dynamic import holds in production. ⚠️ **Do NOT grep for the bare word `firebase`**: `getFirebase` and a destructured `onAuthStateChanged` are call-site code that legitimately ships in the initial chunk, so that needle is a false positive forever |
...
| **PR #13 deployed ↔ HEAD** | `gcloud run services describe` | ✅ tag `f2f20427…` **equals `git rev-parse HEAD`**, deploy green, all 8 routes 200 on both hosts with `s-maxage=300` |
| **Step 6 code is really in prod** | fetch `/admin/intake` chunks, grep rendered copy | ✅ `reabrir su ficha`, `otro animal` and the conflict message all present in the served bundle — 13 chunks, 1389 KB |
| **Deployed rules ↔ `main` after merge** | Rules API readback, **line endings normalised both sides** | ✅ `firestore.rules` and `storage.rules` both identical to `main` |
...
| **Areas + placements, live** | client-SDK probe against live Firestore, admin **and** non-admin | ✅ **22/22, 2026-08-24.** Areas refused to a signed-in non-admin (read, list *and* create); `collectionGroup('placements')` refused to the same account and resolving for an admin; an `intake` placement moving `inbound → quarantine`; the previous stay **closed** with **exactly one** open; the snapshotted `areaName`, `movedBy` and note stored; `merge: true` not dropping `createdAt`; the trace finding the pen-mate and excluding the animal that was never there; `noPlacementData` true for an animal with no placements; `releasePet` closing the stay and opening none |
| **Clock skew is REAL here** | wrote `serverTimestamp()`, read it straight back | ⚠️ **Firestore's clock measured 2.7 s AHEAD of this machine, 2026-08-24.** It broke two things in the new code before anyone shipped them — see the log. `CLOCK_SKEW_TOLERANCE_MS` and a `Math.max(0, …)` clamp are the fixes, and **both have a regression test proven to fail when reverted** |
| **The area/placement tests actually fail when broken** | broke five rules in `areas.ts` on purpose, one at a time | ✅ each break named the test that caught it — **and it found two rules covered by NO test**: an infectious move into a *quarantine* pen, and `statusAfterPlacement`'s legality guard. The first is now tested; the second is unreachable given today's transition table, so the **coupling** is asserted instead and narrowing `PET_STATUS_TRANSITIONS` fails a test by name |
| **Areas UI, end to end in a browser** | create → duplicate rejected → move → warnings → trace → release | ✅ **2026-08-24.** `CUARENTENA  2` rejected against `Cuarentena 2` (case *and* doubled spacing); the ASV + cohorting warnings shown together with the save button **still enabled**; `2 de 2` and *"Último ingreso hoy"*; `0 animalitos` for an uncounted capacity; attribution resolving to the mover's email; `sin área` appearing once a placement was closed. Zero console errors |
| **`revalidate` on a DYNAMIC route** | `curl -I` against a real `next start` | ⚠️ **inert.** `/admin/pets/{id}` **and** the pre-existing `/adopt/[slug]` both answer `private, no-cache, no-store` — the year-long Hosting cache defect is a **static**-page problem. The static routes still measure `s-maxage=300, stale-while-revalidate` |
| Real data | any pet document | ❌ **still none — `pets`, `areas`, `adoptions` all 0 docs** (`users` has 1, the admin). The last gap in step 3, and it needs the **shelter's** animal + photo — but it is now a form at `/admin/intake`, not a JSON file |
| **Probe residue from previous sessions** | Admin SDK count, 2026-08-24 | ⚠️ **found and cleared.** `users` held **3**, not the 1 this file claimed — two orphaned profiles for a deleted `readmit-probe@example.com`, because **deleting an auth account does not cascade to `users/{uid}`**. `petDrafts` held **1** blank draft minted by a wizard reset. Both deleted; readback now 1 user, 0 drafts. *A cleanup "verified at zero" is only as good as the collection list it checked* |
...
From building the arrival pipeline UI (2026-08-24):

- **Firestore's clock is not this machine's clock, and the gap breaks date
  arithmetic silently.** Measured at **2.7 s ahead** on 2026-08-24. Every
  `startedAt` is written with `serverTimestamp()`; every window ceiling and
  every "days since" comes from the caller. So a record written seconds ago is
  routinely in the caller's **future** — which made `Math.floor` produce
  `-1 días`, and made the outbreak trace **return zero contacts and present it
  as an answer**. Any code comparing a `serverTimestamp()` field against
  `Date.now()` needs `CLOCK_SKEW_TOLERANCE_MS` or a clamp. The tolerance is in
  **minutes** because a browser clock is what asks.
- **A warning that blocks gets worked around; decide which is which and say
  why.** Over-capacity, an infectious move into a shared pen, and a restarted
  quarantine clock all WARN and leave the save button enabled — the manager is
  deciding with an animal in their arms. Compare the chip-conflict gate in the
  intake wizard, which genuinely blocks, because proceeding there writes one
  credential onto two records.
- **Two area rows for one pen is a silent data disaster**, which is why the
  name check folds case, accents and inner whitespace. Occupancy splits across
  both rows, so a full pen reads as half full, and an outbreak trace filtered
  by `areaId` misses everyone filed under the other spelling. A human creates
  this by typing.
- **Areas are never deleted, only deactivated.** Every placement references an
  `areaId`, and deleting the area leaves the trace filtering on an id that
  resolves to nothing — it still returns rows, with no name for the pen they
  were in.
- **`export const revalidate` does nothing on a dynamic route.** Measured:
  `/admin/pets/{id}` and `/adopt/[slug]` both answer
  `private, no-cache, no-store` from a real `next start`. The year-long
  Hosting cache defect is a **static**-page problem. Keep the line for the day
  the route prerenders, but do not claim it is what protects the page.
- **⚠️ `git diff CLAUDE.md` reports a WHOLE-FILE REWRITE the moment you touch
  this file, and it is a lie.** `CLAUDE.md`'s committed blob is **CRLF** while
  `core.autocrlf=true` normalises the working tree to **LF** before comparing,
  so every one of its ~1700 lines mismatches. It reads clean until the first
  edit only because `git status` trusts stat info and never compares content.
  The real diff needs one flag:

  ```bash
  git diff --stat --ignore-cr-at-eol CLAUDE.md
  ```

  On 2026-08-24 that turned a reported `3393 lines changed` into the true
...
- **Python's text-mode read turns CRLF into LF**, so every `io.open(p).read()`
  ...
  afterwards. Convert back when done — and note the useful side effect: a file
  showing as *modified with no content diff* is proof a break-probe restore was
  byte-clean.
- **Deleting an auth account does NOT delete `users/{uid}`.** Two orphaned
  profile documents from the 2026-08-23 probes survived a cleanup this file
  recorded as "read back at zero" — the readback simply never counted `users`.
  Firestore has no referential integrity (see `## Architecture` weakness 3);
  the cascade is yours to write.
...
- **⚠️ `gemini_api_key_enabled` is TRUE only in gitignored `terraform.tfvars`.**
  Applied 2026-08-27. A fresh clone copies `terraform.tfvars.example`, where it
  is `false`, so `terraform apply` from another machine plans to **REMOVE** the
  `GEMINI_API_KEY` env binding from Cloud Run — silently un-configuring the AI
  features with no error and a diff that looks deliberate. Check that variable
  before applying anywhere new, and read a plan proposing to drop that `env`
  block as a missing tfvars entry rather than as intended drift. This is the
  inverse of the rule directly below: there, a var Terraform did not declare got
  deleted; here, a var Terraform declares but cannot see the value of does.
