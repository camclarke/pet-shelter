...
is BUILT, PROVEN, MERGED (PR #13, `f2f2042`) and DEPLOYED.** The intake
wizard now looks a chip up before it trusts the form. The deployed Cloud Run
tag equals `git rev-parse HEAD`, all eight routes serve 200 on both hosts,
and the new copy was found in the production bundle — not merely inferred
from a green pipeline.

**Build-order steps 1–6 are done. `pets` still holds 0 documents, and that
is the whole remaining gap in step 3.** It is no longer a technical one: the
