...
| Muro de Adopción | ✅ Server-rendered — `src/components/Muro.tsx`, fetched via `dogs-server.ts` |
...
- **The SEO gap this closes:** the Astro version rendered the public teaser
  ...
  after the crawl, not before it. `dogs-server.ts` uses the Admin SDK inside
