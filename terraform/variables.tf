variable "project_id" {
  description = "GCP project ID. A new dedicated project per the project log — never the trustcert-ai-g work project."
  type        = string
}
...
variable "region" {
  description = <<-EOT
    ...
    earlier note in the project log claimed us-east1 was not — that was inherited
...
variable "budget_amount_usd" {
  description = "Monthly budget alert threshold in USD. The project log commits to an alert at $5 going up before anything is deployed."
  type        = number
  default     = 5
}
