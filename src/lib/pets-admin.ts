 ...
 * principle in the project log's `## Architecture`. Reads and writes here go
...
// The EXIF-stripping pipeline lives in `image-strip.ts`, shared with profile
// photos. Re-exported so every existing caller keeps its import path.
export { PhotoUnreadableError, stripAndResize } from './image-strip';
