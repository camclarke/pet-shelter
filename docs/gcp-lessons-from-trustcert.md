...
2026-08-08 — its project log (43 numbered gotchas), `qa/postmortems/` (three
...
> The rule in the project log still stands — shelter data must
...
the project log's verified-state table as "not yet falsified," not "working."
...
see the project log § "Next session". Steps 5–8 are lifted from what worked there.
...
the project log § Terraform). It has **not** yet reckoned with
rows 3 and 4.
...
- **Maps is this project's equivalent line item** (the project log concern #4): 10k
...
- **Blameless postmortems in the repo.** Their `qa/postmortems/` has a template
  ...
  the reason their project log is worth 163 KB.
...
- **The scale of their project log.** 43 gotchas and five status blocks is a
...
- **Cloud Tasks, Cloud Scheduler, OIDC workers, batch pipelines.** None of it is
  justified here yet. The project log's cost principle — *read Firestore directly
