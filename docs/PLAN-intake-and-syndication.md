...
> | | |
> |---|---|
> | **0. Firebase project** | ❌ **NEW BLOCKER, under #1.** The GCP project was never added to Firebase at all. Needs a human to accept the Firebase ToS in the console — `addFirebase` 403s even for `roles/owner`. See the project log |
...
the project log's queue. This plan does not replace them, it sits on top of them:
...
the project log names repeatedly: a plan that validates is not a plan that works.
...
the project log holds throughout: **a new visibility tier is a new document, never a
new field.**
...
Manager, bound to the Cloud Run runtime service account. The project log already
...
The project log's primary objective is *"get a stranger from scrolling to messaging
...
| Change | Where | Note |
|---|---|---|
| **Secret Manager: `GEMINI_API_KEY`** | `terraform/` + Cloud Run `secret_key_ref` | **Create the secret version BEFORE a `version = "latest"` binding resolves**, or the revision fails to start. The project log already carries this lesson |
...
fewer apply-time API failure, and it sidesteps the ADC hazard the project log
...
the project log — that the undeployed `storage.rules` is not an exposure — rests on
...
2. **Storage: adopt a Firebase default bucket, or keep Terraform-managed
   `wawitas-app`?** Forced by step 4. Inherited from the project log and now due.
3. **What does the adoption application actually ask?** The shelter has real
   ...
   inventing one.
4. **Scan-history retention** — still open from the project log concern #3, and now
...
scan ledger (the project log concern #3) it is **not surveillance of a person** — it
...
the project log's standing warning here: the microchip lookup broke because a
...
- **No BigQuery.** The project log decided 2026-08-09 to add it when a named report
